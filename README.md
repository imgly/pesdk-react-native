...
2. Add the img.ly repository and plugin by opening the `android/build.gradle` file (**not** `android/app/build.gradle`) and adding these lines at the top:
   ...
   buildscript {
       ...
       dependencies {
           ...
           classpath 'ly.img.android.sdk:plugin:8.2.0'
       }
   }
   ```
   In order to update PhotoEditor SDK for Android replace the version string `8.2.0` with a [newer release](https://github.com/imgly/pesdk-android-demo/releases).
